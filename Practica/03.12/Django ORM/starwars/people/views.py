...
def get_data():
    ...
    return data
# обработчик для всех персонажей
def get_all_character(request):
    ...
    return render(request, "people/all_characters.html", {"characters": character_list})
# обработчик для одного персонажа
def get_character(request, id):
    data = get_data()
    pers = data[str(id)]

    return render(request, "people/character.html", pers)